...
- *Inbound*: replies sent to the bot on Telegram arrive via webhook. The webhook looks up the sender's linked account, resolves a direct conversation from the replied-to outbound message or one unambiguous outbound relay-log correlation, and rejects uncorrelated or ambiguous messages rather than selecting an arbitrary coworker. It inserts a real row into `messages` and records the mapping in `telegram_relay_log`. The new message then shows up in the normal chat UI in real time via the existing Realtime subscription — there is no separate "Telegram inbox" data path anymore.
