  ...
  useEffect(() => {
    connectionManager
      ...
      .then(cfg => {
        ...
        try {
          setServerLabel(new URL(cfg.serverUrl).host);
        } catch {
          setServerLabel(cfg.serverUrl);
        }
      })
      .catch(() => setServerLabel(''));
  }, []);
    ...
    <div className="h-full w-full flex bg-gray-100 overflow-hidden">
      ...
      <main className="flex-1 flex items-center justify-center p-6 overflow-y-auto">
        <div className="w-full max-w-[400px] space-y-7">
          ...
          <div className="flex items-center justify-between gap-3 rounded-[10px] border border-gray-200 bg-white/70 px-3 py-2 text-xs text-gray-500">
            <span className="flex items-center gap-2 min-w-0">
              <Server className="h-3.5 w-3.5 shrink-0 text-gray-400" />
              <span className="truncate">Сервер: <span className="font-medium text-gray-700">{serverLabel || '…'}</span></span>
            </span>
            <button type="button" onClick={() => setServerDialogOpen(true)} className="shrink-0 font-semibold text-blue-700 hover:text-blue-800">
              Изменить
            </button>
          </div>
